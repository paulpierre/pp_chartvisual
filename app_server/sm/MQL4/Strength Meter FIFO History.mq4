...
extern      string               URL                              = "http://www.topforexsignal.com/pipsguy/sm";
...
extern 		string  		         •••Strength_Meter_Settings•••    = "••••••••••••••••••••••••";
input       int                  Period1                          = 7;                         //Period
//*------------------------------------------------------------------*
//#define     IndiName    "PipFinite Strength Meter 1.0_source"
#define     IndiName    "Market\\PipFinite Strength Meter"
string      Page = URL + "/sm.php";
//*------------------------------------------------------------------*
...
//*------------------------------------------------------------------*
string GetStrengthMeter(int tf, int bar=0)
{
   int back = BarsCount;
   if(tf==PERIOD_D1) back = D1_BarsCount;
   if(tf==PERIOD_W1) back = W1_BarsCount;
   if(tf==PERIOD_MN1) back = MN1_BarsCount;
   double val = iCustom(Symbol(),tf,IndiName,"",Period1,back,"",clrLightBlue,clrYellowGreen,clrLimeGreen,clrBlue,clrPlum,
   clrHotPink,clrOrangeRed,clrMagenta,"",false,false,8,"",false,false,false,false,0,bar);
   if(val==0) val = iCustom(Symbol(),tf,IndiName,"",Period1,back,"",clrLightBlue,clrYellowGreen,clrLimeGreen,clrBlue,clrPlum,
   clrHotPink,clrOrangeRed,clrMagenta,"",false,false,8,"",false,false,false,false,1,bar);  
   return(IntegerToString(int(val)));
}
//*------------------------------------------------------------------*
void GetVals(ENUM_TIMEFRAMES tf, string& back[])
{
   string val = GetStrengthMeter(tf);
   if(StringToInteger(val)>0)
   {
      back[BULL] = val;
      back[BEAR] = "0";
   }
...
{    
   ...
   string post = StringConcatenate("pair=", Symbol(), "&tf=", PeriodToText(timeframe), "&bear=", vals[BEAR], "&bull=", vals[BULL], "&time=", vals[TIME]);
...
string Get1KBars(int Timeframe, int bars_count)
{
   ...
   string val, bull, bear;
   //for(int cnt=bars_count-1; cnt>=0; cnt--)
   for(int cnt=0; cnt<bars_count; cnt++)
   {
      val = GetStrengthMeter(Timeframe, cnt);
      if(StringToInteger(val)>0) { bull=val; bear="0"; }
      else { bear=val; bull="0"; }
      line = StringConcatenate("INSERT INTO `",Symbol(),"_",PeriodToText(Timeframe),"`(bull,bear,time) VALUES(",
      "'",bull,"',",
      "'",bear,"',",
...
string CreateHeader(string filename)
{
   ...
   text += "`bull` varchar(2) NOT NULL,\n";
   text += "`bear` varchar(2) NOT NULL,\n";
