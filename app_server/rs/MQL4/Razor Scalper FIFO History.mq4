//*------------------------------------------------------------------*
//|                                   Razor Scalper FIFO History.mq4 |
...
#property description "Razor Scalper FIFO History"
...
extern      string               URL                              = "http://www.topforexsignal.com/pipsguy/rs";
...
//*------------------------------------------------------------------*
#define     IndiName    "Market\\PipFinite Razor Scalper"
string      Page = URL + "/rs.php";
//*------------------------------------------------------------------*
...
//*------------------------------------------------------------------*
string GetRazorScalper(int tf, int buf, int bar=0)
{
   int back = BarsCount;
   ...
   double val = iCustom(Symbol(),tf,IndiName,"",Period1,back,"",1,"",false,false,false,
   buf,bar);   
...
{
   back[BUY] = GetRazorScalper(tf, BUY_BUF);
   back[SELL] = GetRazorScalper(tf, SELL_BUF);
   back[BACKGROUND] = GetRazorScalper(tf, BACKGROUND_BUF);
...
{ 
   ...
   string post = StringConcatenate("pair=", Symbol(), "&tf=", PeriodToText(timeframe), "&buy=", vals[BUY], "&sell=", vals[SELL], 
   "&bg=", vals[BACKGROUND], "&time=", vals[TIME]);
...
string Get1KBars(int Timeframe, int bars_count)
{
   ...
   {
      line = StringConcatenate("INSERT INTO `",Symbol(),"_",PeriodToText(Timeframe),"` (`buy`,`sell`,`bg`,`time`) VALUES(",
      "'",GetRazorScalper(Timeframe,BUY_BUF,cnt),"',",
      "'",GetRazorScalper(Timeframe,SELL_BUF,cnt),"',",
      "'",GetRazorScalper(Timeframe,BACKGROUND_BUF,cnt),"',",
...
{  
   ...
   text += "`buy` varchar(15) NOT NULL,\n";
   text += "`sell` varchar(15) NOT NULL,\n";
   text += "`bg` varchar(10) NOT NULL,\n";
