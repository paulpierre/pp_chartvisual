<?php
	...
	{
		//update
		$result = mysql_query("UPDATE $table SET bull='$bull', bear='$bear', buy='$buy', sell='$sell', av='$av', mm='$mm', time='$time' WHERE id='1'") 
		or die(mysql_error());
		bye(UPDATED);		
	}
	else
	{
		...
		for($cnt=2; $cnt<$records_count; $cnt++)
		{
			...
			$ntime = $times[$cnt-2];
			$result = mysql_query("UPDATE $table SET bull='$nbull', bear='$nbear', buy='$nbuy',	sell='$nsell', av='$nav', mm='$nmm', time='$ntime' 
			WHERE id='$cnt'");
		}
		mysql_free_result($result);
