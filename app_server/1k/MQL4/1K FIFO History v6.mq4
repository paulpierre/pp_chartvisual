...
#property description "1K FIFO History"
...
enum YN {No,Yes};
//*------------------------------------------------------------------*
extern      string               URL                              = "http://www.topforexsignal.com/pipsguy/1k";
...
//*------------------------------------------------------------------*
string Page = URL + "/1k.php";
//#define     Page        "http://localhost/aaron/1K_PRICES/1k.php"
...
//*------------------------------------------------------------------*
void GetVals(ENUM_TIMEFRAMES timeframe, string& back[])
{
   back[CLOSE] = DoubleToString(iClose(Symbol(),timeframe,0),Digits);
   back[HIGH] = DoubleToString(iHigh(Symbol(),timeframe,0),Digits);
   back[LOW] = DoubleToString(iLow(Symbol(),timeframe,0),Digits);
   back[OPEN] = DoubleToString(iOpen(Symbol(),timeframe,0),Digits);
   back[TIME] = TimeToString(iTime(Symbol(),timeframe,0));
}
//*------------------------------------------------------------------*
void PostData(ENUM_TIMEFRAMES timeframe)
{
   ...
   string post = StringConcatenate("pair=", Symbol(), "&tf=", PeriodToText(timeframe), 
   "&open=", vals[OPEN], "&close=", vals[CLOSE], "&high=", vals[HIGH], "&low=", vals[LOW], "&time=", vals[TIME]);
...
string Get1KBars(int Timeframe, int bars_count)
{
   ...
   {
      line = StringConcatenate("INSERT INTO `",Symbol(),"_",PeriodToText(Timeframe),"`(`open`,`high`,`close`,`low`,`time`) VALUES(",
      "'",DoubleToStr(iOpen(Symbol(),Timeframe,cnt),Digits),"',",
      "'",DoubleToStr(iHigh(Symbol(),Timeframe,cnt),Digits),"',",
      "'",DoubleToStr(iClose(Symbol(),Timeframe,cnt),Digits),"',",
      "'",DoubleToStr(iLow(Symbol(),Timeframe,cnt),Digits),"',",
...
string CreateHeader(string filename)
{
   ...
   text += "`open` varchar(30) NOT NULL,\n";
   text += "`high` varchar(30) NOT NULL,\n";
   text += "`close` varchar(30) NOT NULL,\n";
   text += "`low` varchar(30) NOT NULL,\n";
...
{
	string FileName = Symbol()+"_"+PeriodToText(Timeframe)+".sql";
