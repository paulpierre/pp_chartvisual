//*------------------------------------------------------------------*
//|                                  Blue Butterfly FIFO History.mq4 |
...
#property description "Blue Butterfly FIFO History"
...
extern      string               URL                              = "http://www.topforexsignal.com/pipsguy/bb";
...
//*------------------------------------------------------------------*
#define     IndiName    "Market\\Blue Butterfly"
string      Page = URL + "/bb.php";
//*------------------------------------------------------------------*
...
//*------------------------------------------------------------------*
string GetBlueButterfly(int tf, int buf, int bar=0)
{
   int back = BarsCount;
...
{
   back[CORNER] = GetBlueButterfly(tf, 0);
...
{ 
   ...
   string post = StringConcatenate("pair=", Symbol(), "&tf=", PeriodToText(timeframe), "&corner=", vals[CORNER], "&time=", vals[TIME]);
...
string Get1KBars(int Timeframe, int bars_count)
{
   ...
   {
      line = StringConcatenate("INSERT INTO `",Symbol(),"_",PeriodToText(Timeframe),"` (`corner`,`time`) VALUES(",
      "'",GetBlueButterfly(Timeframe,0,cnt),"',",
...
{  
   ...
   text += "`corner` varchar(15) NOT NULL,\n";
